...
export class RecyclebinComponent implements OnInit {
  ...
  page: number;
  code: number = 100;
  dataUrl: string = 'admin/trashed';
  admins: object;
  routerUrl: string;
  destroyUrl: string = 'admin/force/delete/';
  destroyMultipleUrl: string = 'admin/force/delete/multiple';
  destroy: destroy;
  checkbox: checkbox;
    ...
    this.http.get(api(this.dataUrl)).subscribe(data => {
      if (data['code'] === 200) {
        ...
        for (let i = 0; i < length; i++) {
          this.admins['data'][i]['index'] = i;
          this.admins['data'][i].checked = false;
        }
        this.destroy = new destroy(this.dialog, this.http, this.admins);
        this.checkbox = new checkbox(this.admins);
  ...
  restoreAction(id, index): void {
    this.http.put(api('admin/restore/' + id), {}).subscribe(data => {
      if (data['code'] === 200) {
