...
export class RecyclebinComponent implements OnInit {
  page: number;
  code: number = 100;
  dataUrl: string = 'comment/trashed';
  comments: object;
  routerUrl: string;
  destroyUrl: string = 'comment/force/delete/';
  destroyMultipleUrl: string = 'comment/force/delete/multiple';
  destroy: destroy;
  checkbox: checkbox;
  ...
  restoreAction(id, index): void {
    this.http.put(api('comment/restore/' + id), {}).subscribe(data => {
      if (data['code'] === 200) {
