...
export class RecyclebinComponent implements OnInit {
  ...
  page: number;
  code: number = 100;
  dataUrl: string = 'contribution/trashed';
  contributions: object;
  routerUrl: string;
  destroyUrl: string = 'contribution/force/delete/';
  destroyMultipleUrl: string = 'contribution/force/delete/multiple';
  destroy: destroy;
  checkbox: checkbox;
  ...
  restoreAction(id, index): void {
    this.http.put(api('contribution/restore/' + id), {}).subscribe(data => {
      if (data['code'] === 200) {
        this.contributions['data'].splice(index, 1);
